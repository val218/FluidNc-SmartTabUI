...
static void init_capacitive_cyd() {
    ...
    // Step size switch (pos2=1mm on 16, pos3=10mm on 4, pos1=NC = 0.1mm)
    pinMode(GPIO_NUM_16, INPUT_PULLUP);
    pinMode(GPIO_NUM_4,  INPUT_PULLUP);

    // Axis select switch (pos2=X on 17, pos3=Y on 35, pos4=Z on 21, pos5=A on 22, pos1=NC = off)
    // GPIO35 is input-only, no internal pullup - use external pullup or accept floating
    pinMode(GPIO_NUM_17, INPUT_PULLUP);
    pinMode(GPIO_NUM_35, INPUT);          // input-only pin, no pullup available
    // GPIO_NUM_21 and GPIO_NUM_22 shared with encoder — PCNT configures them,
    // don't re-pinMode or it will break the encoder

    // E-stop button (active low)
    pinMode(GPIO_NUM_8, INPUT_PULLUP);

    // Keep old button pin vars set so switch_button_touched still compiles
    red_button_pin   = -1;
    dial_button_pin  = -1;
    green_button_pin = -1;
}
