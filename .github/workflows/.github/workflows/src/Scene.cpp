...
void dispatch_events() {
    ...
    // E-stop: GPIO8 active-low — send realtime reset
    {
        static bool lastEstop = true;
        static uint32_t estopLast = 0;
        uint32_t now = millis();
        if (now - estopLast >= 20) {   // debounce 20ms
            estopLast = now;
            bool estopHigh = (gpio_get_level(GPIO_NUM_8) != 0);
            if (!estopHigh && lastEstop) {   // falling edge = pressed
                fnc_realtime(Reset);
            }
            lastEstop = estopHigh;
        }
