...
private:
    ...
    Rect _spndMinus, _spndPlus, _spndPill;
    ...
    void drawHeader() {
        ...
        // Right-side: TX / RX text only (no circles), EN text, E-STP badge
        // Layout from right: RX(W-4) TX(W-22) EN(W-40) [STP badge if active]
        {
        ...
        // E-stop badge (only when active) — sits left of EN
        if (mpgEstopActive) {
            fillR(W-74, 2, 32, 16, 3, 0x4000);
            strokeR(W-74, 2, 32, 16, 3, RED);
            hdrTxt("STP", W-58, 10, RED);
        }

        // MPG direction arrows — drawn RIGHT of EN, replacing TX/RX area when spinning
        // Show for 300ms after wheel movement — two small triangles ◄ ►
        if (mpgLastDir != 0 && (millis() - mpgDirTime) < 300) {
            ...
            int dimCol = COL_DIM;
            // Draw arrows over the RX/TX area (rightmost 24px)
            canvas.fillRect(W-25, 2, 23, 16, COL_PANEL);  // clear text behind arrows
            if (mpgLastDir < 0) {
                drawTri(W-18, false, ac);     // ◄ bright
                drawTri(W-6,  true,  dimCol); // ► dim
            } else {
                drawTri(W-18, false, dimCol); // ◄ dim
                drawTri(W-6,  true,  ac);     // ► bright
            }
        }

        // JOG label — shown over tab name when enable held and axis selected
    ...
    void drawDROScreen() {
        ...
        // ── Feed + Spindle control bar — tap pill to select, MPG adjusts ─────
        int fy  = NAV_Y - FEED_H;
        canvas.fillRect(0, fy, W, FEED_H, COL_PANEL2);
        hline(0, fy, W, COL_BORDER);

        auto barTxt = [&](const char* s, int x, int y, int col, int datum = middle_center) {
            canvas.setFont(&fonts::Font2);
            canvas.setTextDatum(datum);
            canvas.setTextColor(col);
            canvas.drawString(s, x, y);
        ...
        int pad2 = 6, mid = W / 2;
        int pillW2 = mid - pad2 - 3;  // each pill: pad → mid-3, 3px gap, mid+3 → W-pad
        int fh = FEED_H - 8, fy2 = fy + 4;

        // ── FEED pill (left) ─────────────────────────────────────────────────
        int fc = (myFro < 80) ? RED : (myFro > 120) ? ORANGE : CYAN;
        bool feedSel = (_barSel == 1);
        _feedMinus = { pad2, fy, pillW2, FEED_H };  // repurpose as feed pill rect
        if (feedSel) {
            canvas.fillRoundRect(pad2, fy2, pillW2, fh, 4, fc);
            canvas.drawRoundRect(pad2, fy2, pillW2, fh, 4, COL_WHITE);
            barTxt("FEED", pad2 + pillW2/2, fy + FEED_H*32/100, COL_BG);
            char fpct[10]; snprintf(fpct, sizeof(fpct), "%d%%", (int)myFro);
            barTxt(fpct, pad2 + pillW2/2, fy + FEED_H*70/100, COL_BG);
        } else {
            tintStrokeR(pad2, fy2, pillW2, fh, 4, fc, fc, 18);
            barTxt("FEED", pad2 + pillW2/2, fy + FEED_H*32/100, fc);
            char fpct[10]; snprintf(fpct, sizeof(fpct), "%d%%", (int)myFro);
            barTxt(fpct, pad2 + pillW2/2, fy + FEED_H*70/100, COL_WHITE);
        }

        // Divider
        vline(mid, fy + 4, fh, COL_BORDER2);

        // ── SPINDLE pill (right) ─────────────────────────────────────────────
        int sc = (mySro < 80) ? RED : (mySro > 120) ? ORANGE : 0xF81F;
        bool spndSel = (_barSel == 2);
        int spx = mid + 3;
        _spndPill = { spx, fy, pillW2, FEED_H };
        if (spndSel) {
            canvas.fillRoundRect(spx, fy2, pillW2, fh, 4, sc);
            canvas.drawRoundRect(spx, fy2, pillW2, fh, 4, COL_WHITE);
            barTxt("SPND", spx + pillW2/2, fy + FEED_H*32/100, COL_BG);
            char spct[10]; snprintf(spct, sizeof(spct), "%d%%", (int)mySro);
            barTxt(spct, spx + pillW2/2, fy + FEED_H*70/100, COL_BG);
        } else {
            tintStrokeR(spx, fy2, pillW2, fh, 4, sc, sc, 18);
            barTxt("SPND", spx + pillW2/2, fy + FEED_H*32/100, sc);
            char spct[10]; snprintf(spct, sizeof(spct), "%d%%", (int)mySro);
            barTxt(spct, spx + pillW2/2, fy + FEED_H*70/100, COL_WHITE);
        }
    }
...
public:
    ...
    void onEncoder(int delta) override {
        ...
        if (_tab == 0 && _barSel != 0) {
            if (_barSel == 1) {
                // Feed override ±10% per detent
                fnc_realtime(delta > 0 ? FeedOvrCoarsePlus : FeedOvrCoarseMinus);
            } else {
                // Spindle override ±10% per detent (0x9A=+10%, 0x9B=-10%)
    ...
    void onTouchClick() override {
        ...
        if (_tab == 0) {
            ...
            if (y >= NAV_Y - FEED_H) {
                if (x < W/2) { _barSel = (_barSel==1) ? 0 : 1; reDisplay(); return; }
                else          { _barSel = (_barSel==2) ? 0 : 2; reDisplay(); return; }
            }
            // Viz tabs
            int tabH = VIZ_H / 3;
            if (x >= TAB_X && x <= W && y >= VIZ_Y && y < NAV_Y - FEED_H) {
                int ti = (y - VIZ_Y) / tabH;
