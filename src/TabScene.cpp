...
private:
    ...
    void drawHeader() {
        ...
        // Fixed slots from right edge, each properly centred:
        //   RX : bg W-20..W-2  text centred W-11
        //   TX : bg W-40..W-22 text centred W-31
        //   ◄► : W-58..W-43
        //   EN : right-edge W-60
        {
            ...
            // RX — rightmost, bg then text centred in same rect
            ...
            // TX — next slot left
            if(txOn) canvas.fillRoundRect(W-40,3,18,14,2,0x3200);
            canvas.setTextColor(txOn?ORANGE:COL_DIM);
            canvas.drawString("TX", W-31, 10);

            // ◄► direction arrows — 14px wide slot, 4px gap left of TX
            {
                ...
                // Two 6px triangles centred in W-58..W-44
                auto drawTri = [&](int cx2, bool pointRight, int col) {
                    for (int r=0;r<7;r++){
                        int h=(r<4)?r:(6-r);
                        int lx=pointRight?(cx2-h/2):(cx2-3+h),
                            rx2=pointRight?(cx2-h/2+h):(cx2+3);
                        if(rx2>lx) canvas.drawFastHLine(lx,3+r,rx2-lx+1,col);
                    }
                };
                canvas.fillRect(W-58,2,15,16,COL_PANEL);
                drawTri(W-53, false, active&&mpgLastDir<0 ? ac : COL_DIM);  // ◄
                drawTri(W-47, true,  active&&mpgLastDir>0 ? ac : COL_DIM);  // ►

            }

            // EN — right edge at W-62, 4px gap from arrows
            hdrTxt("EN", W-62, 10, mpgEnable?GREEN:COL_DIM, middle_right);
        }
    ...
    void drawGcodePreview() {
        ...
        if (_confirmRun) {
            canvas.fillRoundRect(12+2*bw3,by3,bw3,bh3,3,GREEN);
            f2("CONFIRM?",12+2*bw3+bw3/2,abY+abH2/2,COL_BG,middle_center);
        } else {
    ...
    void drawFilesScreen() {
        ...
        if(fileSelected>=0&&fileSelected<(int)fileList.size()&&!fileList[fileSelected].isDir){
            ...
            if (_confirmRun) {
                canvas.fillRoundRect(8+btnW,btnY,btnW,btnH,4,GREEN);
    ...
    void drawMacrosScreen() {
        ...
        for(int vi=0;vi<visible&&(macroScroll+vi)<totalItems;vi++){
            ...
            if(itemIdx==0){
                ...
                canvas.fillRoundRect(pad,by,bw,rh,3,COL_PANEL2);
                canvas.drawRoundRect(pad,by,bw,rh,3,0xF81F);
                canvas.fillRect(pad+1,by+2,4,rh-4,0xF81F);
                // Icon
                canvas.setFont(&fonts::Font0); canvas.setTextDatum(middle_center);
                canvas.setTextColor(0xF81F);
                canvas.drawString("S",pad+12,by+rh/2);
                // Name + sub in single row (28px — use two lines via y offset)
                canvas.setFont(&fonts::Font2); canvas.setTextDatum(middle_left);
                canvas.setTextColor(COL_WHITE);
                canvas.drawString("Spindle / Coolant",pad+22,by+rh*35/100);
                canvas.setFont(&fonts::Font0); canvas.setTextDatum(middle_left);
                canvas.setTextColor(COL_DIM2);
                canvas.drawString("CW  CCW  Stop  Flood",pad+22,by+rh*72/100);
                canvas.setTextDatum(middle_right);
                canvas.setTextColor(0xF81F);
                ...
                if(isP6){ canvas.fillRoundRect(pad,by,bw,rh,3,0x0019); canvas.drawRoundRect(pad,by,bw,rh,3,mcol);}
                else{ canvas.fillRoundRect(pad,by,bw,rh,3,COL_PANEL2); canvas.drawRoundRect(pad,by,bw,rh,3,COL_BORDER2);}
                canvas.fillRect(pad+1,by+2,4,rh-4,mcol);
        ...
        if (_tab == 2) {
            ...
            if (y >= abY && fileSelected>=0 && fileSelected<(int)fileList.size()
                ...
                if (y>=btnY && y<btnY+btnH) {
                    if (x<=4+btnW) {
                        // View button — always open G-code text first
                        previewScroll=0; _previewShowPath=false;
                        filePreviewMode=true; markDirty(); return;
                    } else if (x>=8+btnW) {
                        // Run button — requires confirmation
                        if (!_confirmRun) {
                            _confirmRun = true;  // show confirm state
                            markDirty(); return;
                        }
                        // Confirmed — send to FluidNC
                        _confirmRun = false;
                ...
                _confirmRun = false;  // cancel confirm if tapped outside buttons
                return;
            }
            // File row taps
            int maxR=(abY-listY)/rowH;
            ...
            if (filePreviewMode) {
                ...
                if (y >= abY) {
                    _confirmRun = false;  // any action bar tap resets confirm
                    if (x < 4+bw3) {
