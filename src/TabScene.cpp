...
private:
    ...
    void drawGcodePreview() {
        ...
        int abY = NAV_Y - 22;
        canvas.fillRect(0, abY, W, 22, COL_PANEL);
        hline(0, abY, W, COL_BORDER);
        char info[48];
        snprintf(info, sizeof(info), "%d lines", total > 0 ? total : 0);
        f2(info, 8, abY + 11, COL_DIM2, middle_left);
        tintStrokeR(W - 48, abY + 3, 44, 16, 2, GREEN, GREEN, 40);
        f2("Run", W - 26, abY + 11, GREEN);
    }
            ...
            if (filePreviewMode) {
                ...
                if (y >= NAV_Y - 22 && x >= W - 48) {
                    if (!simMode_active()) {
                        ...
                        termLines.push_back({ "> [SIM] Run: " + fileList[fileSelected].name, ORANGE });
                    }
                    _tab = 3; filePreviewMode = false; reDisplay(); return;
                }
