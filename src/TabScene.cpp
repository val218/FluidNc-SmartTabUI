...
private:
    ...
    void drawDROScreen() {
        ...
        // ── Hold action strip: 3 mode buttons ─────────────────────────────────
        // Z Adjust | Rewind (path retrace) | Free Jog
        if (state == Hold && !_pathJogMode && !_zNudgeOpen && !_freeJogMode && _tab == 0) {
            int stripH = 32, stripY = VIZ_Y + VIZ_H - stripH;
            int gap = 3, bh = 22, by = stripY + 5;
            int bw = (VIZ_W - gap*4) / 3;
            int bx1 = VIZ_X + gap;
            int bx2 = bx1 + bw + gap;
            int bx3 = bx2 + bw + gap;
            ...
            canvas.drawString((_retraceSupport == 2) ? "No plug" : "Rewind",
                              bx2+bw/2, by+bh/2);
            // Free Jog
            dropShadow(canvas, bx3, by, bw, bh, 3, 1, 1);
            gradientRoundRect(canvas, bx3, by, bw, bh, 3, 0x0022, 0x0008);
            canvas.drawRoundRect(bx3, by, bw, bh, 3, ORANGE);
    ...
    void onTouchClick() override {
        ...
        if (_tab == 0) {
            ...
            if (x >= VIZ_X && x < VIZ_X+VIZ_W && y >= VIZ_Y && y < VIZ_Y+VIZ_H) {
                ...
                // 3D/2D toggle button tap (top-right, fullscreen mode)
                if (_vizFullscreen && !vizPath.empty() &&
                    x >= VIZ_X+VIZ_W-24 && y >= VIZ_Y+2 && y <= VIZ_Y+14) {
