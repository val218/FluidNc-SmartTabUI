...
static int   mpgLastDir   = 0;      // last jog direction: +1 / -1 / 0
static uint32_t mpgDirTime = 0;     // millis() when last jog fired (for fade-out)

// Sample a floating/driven GPIO to distinguish NC (float), PD (GND), PU (3.3V).
// With no internal pull, a floating pin reads noisily ~50% each way.
// A driven pin reads consistently LOW or HIGH.
// Returns: -1=float(NC), 0=LOW(PD→GND), 1=HIGH(PU→3.3V)
static int samplePin(gpio_num_t pin, int n = 16) {
    int high = 0, low = 0;
    for (int i = 0; i < n; i++) {
        if (gpio_get_level(pin)) high++; else low++;
    }
    if (high >= n * 9 / 10) return 1;   // >90% HIGH  → driven HIGH (PU)
    if (low  >= n * 9 / 10) return 0;   // >90% LOW   → driven LOW  (PD)
    return -1;                           // mixed       → floating    (NC)
}

void readMpgSwitches() {
    static uint32_t lastRead     = 0;
    static int16_t  lastPcnt     = 0;
    static uint32_t lastMoveTime = 0xFFFFFFFF - 2000; // distrust GPIO21/22 on boot

    uint32_t now = millis();
    if (now - lastRead < 80) return;
    lastRead = now;

    // Track encoder movement to decide when GPIO21/22 are safe to read
    int16_t curPcnt = 0;
    pcnt_get_counter_value(PCNT_UNIT_0, &curPcnt);
    if (curPcnt != lastPcnt) { lastPcnt = curPcnt; lastMoveTime = now; }
    bool encoderStill = (now - lastMoveTime) > 300;

    int prevAxis = mpgAxis, prevStep = mpgStepIdx;

    // ── Axis switch ──────────────────────────────────────────────────────────
    // Wiring (all GPIOs plain INPUT, no internal pull):
    //   pos1 NC:             all pins float         → off
    //   pos2 GPIO17 PD:      GPIO17=LOW             → X
    //   pos3 GPIO35 PU+10k:  GPIO35=HIGH            → Y
    //   pos4 GPIO17 PU:      GPIO17=HIGH            → Z
    //   pos5 GPIO4  PD:      GPIO4=LOW              → A
    //
    // GPIO17: LOW=X, HIGH=Z, float=off
    // GPIO35: HIGH=Y (has external 10k pullup), float=off
    // GPIO4:  LOW=A, float=off
    // GPIO21/22: not used for axis — encoder only

    int g17 = samplePin(GPIO_NUM_17);
    int g35 = samplePin(GPIO_NUM_35);
    int g4  = samplePin(GPIO_NUM_4);

    if      (g4  == 0) mpgAxis = 3;   // A: GPIO4 driven LOW
    else if (g17 == 1) mpgAxis = 2;   // Z: GPIO17 driven HIGH
    else if (g35 == 1) mpgAxis = 1;   // Y: GPIO35 driven HIGH (ext pullup)
    else if (g17 == 0) mpgAxis = 0;   // X: GPIO17 driven LOW
    else               mpgAxis = -1;  // off: all floating

    // ── Step switch ──────────────────────────────────────────────────────────
    // Single GPIO16, plain INPUT:
    //   pos1 NC:      GPIO16 float     → 0.1mm
    //   pos2 GPIO16 PD: GPIO16=LOW     → 1mm
    //   pos3 GPIO16 PU: GPIO16=HIGH    → 10mm
    if (mpgAxis >= 0) {
        int g16 = samplePin(GPIO_NUM_16);
        if      (g16 == 1) mpgStepIdx = 2;   // 10mm: driven HIGH
        else if (g16 == 0) mpgStepIdx = 1;   // 1mm:  driven LOW
        else               mpgStepIdx = 0;   // 0.1mm: floating
    }

    // Clear PCNT backlog when axis goes off → active
    if (prevAxis < 0 && mpgAxis >= 0) {
        pcnt_counter_clear(PCNT_UNIT_0);
        lastPcnt = 0; lastMoveTime = 0;
    }
...
private:
    ...
    void drawHeader() {
        ...
        // MPG direction indicator — shown for 600ms after any wheel movement
        if (mpgLastDir != 0 && (millis() - mpgDirTime) < 600) {
            static const int axCols[] = { COL_AX_X, COL_AX_Y, COL_AX_Z, COL_AX_A };
            // Axis selected = axis colour; axis=off = dim white
    ...
    void drawDROScreen() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            canvas.fillRect(0, ry, stripW, DRO_ROW, axcol);
            hline(0, ry + DRO_ROW - 1, DROW, COL_BORDER);

            // Axis letter (top of row)
            txt(AX_STYLES[i].letter, 7, ry + DRO_ROW * 28 / 100, axcol, TINY, middle_left);

            // Position value (middle of row)
            txt(pos_to_cstr(myAxes[i], num_digits()), DROW - 5, ry + DRO_ROW * 58 / 100, COL_WHITE, TINY, middle_right);

            // Step size indicator (bottom of row) — only when this axis is MPG active
            if (isMpg) {
                char stepStr[8];
                snprintf(stepStr, sizeof(stepStr), "%smm", mpgStepLabels[mpgStepIdx]);
                canvas.setFont(&fonts::Font0);
                canvas.setTextDatum(middle_right);
                canvas.setTextColor(axcol);
                canvas.drawString(stepStr, DROW - 5, ry + DRO_ROW * 85 / 100);
            }
        }
...
public:
    ...
    void onEncoder(int delta) override {
        // MPG handwheel — jog when axis selected, scroll UI when axis=off
        if (mpgAxis >= 0) {
        ...
        // Axis=off: wheel scrolls active list/log — still show direction in header
        mpgLastDir = (delta > 0) ? 1 : -1;
        mpgDirTime = millis();
        if (_tab == 2) {
