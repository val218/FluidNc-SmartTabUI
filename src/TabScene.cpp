...
void readMpgSwitches() {
    ...
    // Axis — priority order (only one should be active at a time)
    if      (axA) mpgAxis = 3;
    ...
    if (mpgAxis >= 0) {
        if      (st10) mpgStepIdx = 2;  // 10mm
        else if (st1)  mpgStepIdx = 1;  // 1mm
...
private:
    ...
    void drawDROScreen() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            const char* posStr = simMode_active()
                ? pos_to_cstr(simMode_getPos(i), 2)
                : pos_to_cstr(myAxes[i], num_digits());
            txt(posStr, DROW - 5, ry + DRO_ROW * 58 / 100,
        ...
        // Advance path animation when sim job is running
        if (simJobRunning && !simPath.empty()) {
            simPathIdx = std::min(simPathIdx + 1, (int)simPath.size() - 1);
            // Update sim axis positions to match path
            if (simPathIdx < (int)simPath.size()) {
                // Inject path position into sim axes
                simMode_setPos(0, simPath[simPathIdx].first);
                simMode_setPos(1, simPath[simPathIdx].second);
            }
            if (simPathIdx >= (int)simPath.size()-1) simJobRunning = false;
        }
    ...
    void drawGcodePreview() {
        ...
        // G-code button (already showing - highlight it)
        ...
        // Visualise button
        canvas.fillRoundRect(b2x, abY+3, bw, bh, 3, COL_PANEL2);
        canvas.drawRoundRect(b2x, abY+3, bw, bh, 3, CYAN);
        f2("Visualise", b2x + bw/2, abY+12, CYAN);

        // Run button
        canvas.fillRoundRect(b3x, abY+3, bw, bh, 3, 0x0C00);
        canvas.drawRoundRect(b3x, abY+3, bw, bh, 3, GREEN);
        f2("Run", b3x + bw/2, abY+12, GREEN);
    }
        ...
        if (_tab == 2) {
            ...
            if (filePreviewMode) {
                ...
                if (y >= NAV_Y - 24) {
                    ...
                    int b1x = bp, b2x = bp*2+bw, b3x = bp*3+bw*2;
                    if (x >= b1x && x < b1x+bw) {
                        // G-code — already showing, do nothing
                        return;
                    }
                    if (x >= b2x && x < b2x+bw) {
                        // Visualise — parse gcode path and show in DRO visualizer
                        ...
                        for (auto& ln : previewLines) {
                            ...
                            // Very basic XY extraction
                            if (*p=='G'||*p=='g') {
                                const char* xp = strpbrk(p,"Xx");
                                const char* yp = strpbrk(p,"Yy");
                                if (xp) { nx = strtof(xp+1,nullptr); moved=true; }
                                if (yp) { ny = strtof(yp+1,nullptr); moved=true; }
                            }
                            if (moved) {
                        ...
                        filePreviewMode = false;
                        _tab = 0;   // go to DRO tab to show visualizer
                        reDisplay(); return;
                    }
                    if (x >= b3x && x < b3x+bw) {
                        // Run
                        if (!simMode_active()) {
                            ...
                            _tab = 0;   // go to DRO to show sim running
                        }
                        filePreviewMode = false; reDisplay(); return;
                    }
                    return;
