...
private:
    ...
    void drawHeader() {
        ...
        // E-stop indicator — red when pressed
        if (mpgEstopActive) {
            fillR(W - 108, 2, 36, 16, 3, 0x4000);
            strokeR(W - 108, 2, 36, 16, 3, RED);
            hdrTxt("E-STP", W - 90, 10, RED);
        }
        // Enable button indicator — green when held, dim when not
        int enCol = mpgEnable ? GREEN : COL_DIM;
        canvas.fillCircle(W - 68, 10, 3, enCol);
        hdrTxt("EN", W - 63, 10, enCol, middle_left);

        // TX/RX live activity indicators — flash bright on activity
        {
            static uint32_t lastTx = 0, lastRx = 0;
            static uint32_t txFlash = 0, rxFlash = 0;
            uint32_t now2 = millis();

            if (fnc_tx_count != lastTx) { lastTx = fnc_tx_count; txFlash = now2; }
            if (fnc_rx_count != lastRx) { lastRx = fnc_rx_count; rxFlash = now2; }

            bool txOn = (now2 - txFlash) < 80;
            bool rxOn = (now2 - rxFlash) < 80;

            int txCol = txOn ? ORANGE : COL_DIM;
            int rxCol = rxOn ? GREEN  : COL_DIM;

            canvas.fillCircle(W - 72, 10, 3, txCol);
            hdrTxt("TX", W - 67, 10, txCol, middle_left);
            canvas.fillCircle(W - 52, 10, 3, rxCol);
            hdrTxt("RX", W - 47, 10, rxCol, middle_left);
        }
