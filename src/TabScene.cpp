...
static const int DROW    = 88;  // compact: was 110, saves 22px for VIZ
static const int FEED_H  = 32;  // slim pills: was 46, saves 14px for VIZ
...
private:
    int  _tab      = 0;
    int  _vizView  = 0;   // 0=top-down  1=isometric angle view
    ...
    void drawDROScreen() {
        ...
        for (int i = 0; i < nAxes; i++) {
            ...
            if (_currentTheme == 2) {
                ...
                txt(axLetter, 5, midY, 0x0000, SMALL, middle_left);
            } else {
                txt(axLetter, 5, midY, isMpg ? COL_WHITE : axcol, TINY, middle_left);
            }

            // Position value right-aligned
            int ndig = inInches ? 4 : 3;
            const char* posStr = pos_to_cstr(myAxes[i], ndig);
            // auto_text shrinks font if value overflows DRO column
            // at DROW=88 we have ~55px for the number after axis letter
            canvas.setFont(&fonts::Font0);
            canvas.setTextDatum(middle_right);
            canvas.setTextColor(isMpg ? COL_WHITE : COL_WHITE2);
            canvas.drawString(posStr, DROW - 3, midY);
        ...
        if (_vizFullscreen && !vizPath.empty()) {
            ...
            for(int pi=1;pi<(int)vizPath.size();pi+=step){
                ...
                if(x1>=offX&&x1<offX+drawnW&&y1>=offY&&y1<offY+drawnH)
                    canvas.drawLine(x1,y1,x2,y2,pi<=exec?CYAN:COL_BORDER2);
            }
            ...
            if (_vizView == 1 && !vizPath.empty()) {
                ...
                for (int pi = 1; pi < (int)vizPath.size(); pi += istep) {
                    ...
                    float t = (vizPath[pi].z - zMin) / (zMax - zMin + 0.001f);
                    // Interpolate: blue at bottom (t=0), yellow at top (t=1)
                    uint8_t r = (uint8_t)(t * 248); uint8_t g = (uint8_t)(t * 252);
                    uint8_t b = (uint8_t)((1.0f-t) * 248);
                    uint16_t col = ((r & 0xF8)<<8)|((g & 0xFC)<<3)|(b>>3);
                    if (pi <= exec) col = CYAN;
                    // Clip to viz area
                    if (x1>=VIZ_X&&x1<VIZ_X+VIZ_W&&y1>=VIZ_Y&&y1<VIZ_Y+VIZ_H)
            ...
            // [3D/2D] toggle — left of clear button, always visible in path mode
            {
                const char* vl = (_vizView == 1) ? "2D" : "3D";
                uint16_t vc = (_vizView == 1) ? (uint16_t)GREEN : COL_DIM;
                canvas.fillRoundRect(VIZ_X+VIZ_W-52,VIZ_Y+2,24,18,3,_vizView==1?0x0320:COL_PANEL);
                canvas.drawRoundRect(VIZ_X+VIZ_W-52,VIZ_Y+2,24,18,3,vc);
                canvas.setFont(&fonts::Font2);canvas.setTextDatum(middle_center);
                canvas.setTextColor(vc);
                canvas.drawString(vl,VIZ_X+VIZ_W-40,VIZ_Y+11);
            }
        ...
        int lbl_y = fy + 8;   // slim: top label row
        int val_y = fy + 22;  // slim: bottom value row
    ...
    void onMessage(char* command, char* arguments) override {
        ...
        if (arguments && strncmp(arguments, "VizReady:", 9) == 0) {
            ...
            if (colon) {
                ...
                _vizLoadTotal = (int)n;
                float z0=0,z1=0;
                // Try to parse optional zmin/zmax (new format)
                // VizReady:/sd/file.viz:N:xmin:xmax:ymin:ymax[:zmin:zmax]
    ...
    void onFileLines(int firstLine, const std::vector<std::string>& lines) override {
        ...
        if (_vizLoading && !_vizLoadPath.empty()) {
            ...
            for (auto& l : lines) {
                ...
                float x = 0, y = 0;
                float z = 0;
                int parsed = sscanf(l.c_str(), "%f,%f,%f", &x, &y, &z);
                if (parsed >= 2) {
                    if ((int)vizPath.size() < VIZ_MAX_POINTS)
                        vizPath.push_back({x, y, z});
                }
            }
        ...
        if (firstLine == 0 && !lines.empty() && lines[0].substr(0,4) == "VIZ:") {
            ...
            int expected = 0;
            try { expected = std::stoi(lines[0].substr(4)); } catch(...) {}
            ...
            for (int li = 1; li < (int)lines.size(); li++) {
                ...
                try {
                    ...
                    float z = (comma2 != std::string::npos) ? std::stof(ln.substr(comma2+1)) : 0.0f;
                    vizPath.push_back({x, y, z});
    ...
    void onTouchClick() override {
        ...
        if (_tab == 0) {
            ...
            if (x >= VIZ_X && x < VIZ_X+VIZ_W && y >= VIZ_Y && y < VIZ_Y+VIZ_H) {
                ...
                // 3D/2D toggle button tap (always shown when path loaded)
                if (!vizPath.empty() &&
                    x >= VIZ_X+VIZ_W-52 && x < VIZ_X+VIZ_W-26 &&
                    y >= VIZ_Y+2 && y <= VIZ_Y+20) {
                    _vizView = (_vizView == 0) ? 1 : 0;
                    reDisplay(); return;
                }
                // Clear/close button (x button top-right)
                if (!vizPath.empty() && x >= VIZ_X+VIZ_W-26 && y <= VIZ_Y+21 && !_vizFullscreen) {
                    vizPath.clear(); vizJobName.clear(); vizPathExecuted=0;
                    _vizFullscreen=false; _vizView=0; markDirty(); return;
                }
                uint32_t now2=millis();
            ...
            if (y >= listY && y < abY) {
                ...
                if (fi < (int)fileList.size()) {
                    if (fileList[fi].isDir) {
                        ...
                        vizJobName=fileList[fi].name; vizPathExecuted=0;
