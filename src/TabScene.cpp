...
// Read a pin multiple times and return true only if the majority are LOW.
// Used for GPIO21/22 which are shared with the PCNT encoder.
// Encoder pulses last ~1us; a switch closure lasts the full sample window.
static bool stableLow(gpio_num_t pin, int n = 10) {
    int low = 0;
    for (int i = 0; i < n; i++) {
        if (gpio_get_level(pin) == 0) low++;
        ets_delay_us(20);
    }
    return low >= (n * 7 / 10);  // 70% majority
}
...
void readMpgSwitches() {
    ...
    // ── Axis switch ──────────────────────────────────────────────────────────
    // pos2=GPIO17(X)  clean pin, single read
    // pos3=GPIO35(Y)  input-only, NO internal pullup — requires external 10k to 3.3V
    // pos4=GPIO21(Z)  shared with PCNT encoder B — use stableLow
    // pos5=GPIO22(A)  shared with PCNT encoder A — use stableLow
    // pos1=NC         all above read HIGH → mpgAxis = -1

    bool a17 = (gpio_get_level(GPIO_NUM_17) == 0);
    bool a35 = (gpio_get_level(GPIO_NUM_35) == 0);
    bool a21 = stableLow(GPIO_NUM_21);
    bool a22 = stableLow(GPIO_NUM_22);

    if      (a22) mpgAxis = 3;   // A
    else if (a21) mpgAxis = 2;   // Z
    else if (a35) mpgAxis = 1;   // Y  (needs external pullup or will float)
    else if (a17) mpgAxis = 0;   // X
    else          mpgAxis = -1;  // off

    // ── Step switch ──────────────────────────────────────────────────────────
    // pos2=GPIO16(1mm)   no conflict, clean read
    // pos3=GPIO4(10mm)   no conflict, clean read
    // pos1=NC(0.1mm)
    // Only meaningful when an axis is selected
    if (mpgAxis >= 0) {
