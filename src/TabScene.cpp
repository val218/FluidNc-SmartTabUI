...
void tabui_setTheme(int t) {
    ...
    switch (t) {
        ...
        case 2:  // Light — high contrast light blue, near-black text
            COL_BG=0xEF7F;    // very light blue-white
            COL_PANEL=0xCEFF; // light blue (alternating row A)
            COL_PANEL2=0xB5FE;// medium blue (buttons, selected)
            COL_PANEL3=0xD75F;// slightly different (alternating row B)
            COL_BORDER=0x867D; // visible border
            COL_BORDER2=0x5CDB;// stronger border/accent
            COL_DIM=0x2124;   // dark grey text (labels)
            COL_DIM2=0x0841;  // near-black text (values)
            COL_WHITE=0x0000; // black (primary text)
            COL_WHITE2=0x18C3;// dark blue-grey (secondary text)
            COL_AX_X=0x9000; // dark red X
            COL_AX_Y=0x0500; // dark green Y
            COL_AX_Z=0x9200; // dark orange Z
            COL_AX_A=0x600F; // dark purple A
            break;
...
static void tintStrokeR(int x, int y, int w, int h, int r, int fillCol, int strokeCol, int alpha = 30) {
    ...
    if (_currentTheme == 2) {
        ...
        int a = alpha * 3;  // scale alpha for light blend (more visible)
        if (a > 255) a = 255;
...
private:
    ...
    void drawNav() {
        ...
        for (int i = 0; i < N_TABS; i++) {
            ...
            if (i == _tab) {
                canvas.fillRect(x, NAV_Y, w, BOT, COL_PANEL2);
                canvas.fillRect(x, NAV_Y, w, 3, COL_AX_X);  // accent colour from theme
            }
    ...
    void drawDROScreen() {
        ...
        { int px=DROW+2, py=TOP+2, pw=36, ph=10, g=3;
          // MPos/WPos toggle
          tintStrokeR(px, py, pw, ph, 2, _showMPos?COL_BORDER:COL_PANEL3, _showMPos?COL_DIM:CYAN, 30);
          canvas.setFont(&fonts::Font0); canvas.setTextDatum(middle_center);
          canvas.setTextColor(_showMPos?COL_DIM:CYAN);
          canvas.drawString("WPos", px+pw/2, py+ph/2);
          px+=pw+g;
          tintStrokeR(px, py, pw, ph, 2, _showMPos?COL_PANEL3:COL_BORDER, _showMPos?CYAN:COL_DIM, 30);
          canvas.setTextColor(_showMPos?CYAN:COL_DIM);
          canvas.drawString("MPos", px+pw/2, py+ph/2);
          px+=pw+g+4;
          // mm/in toggle
          tintStrokeR(px, py, 24, ph, 2, inInches?COL_BORDER:COL_PANEL3, inInches?COL_DIM:YELLOW, 30);
          canvas.setTextColor(inInches?COL_DIM:YELLOW);
          canvas.drawString("mm", px+12, py+ph/2);
          px+=27;
          tintStrokeR(px, py, 24, ph, 2, inInches?COL_PANEL3:COL_BORDER, inInches?YELLOW:COL_DIM, 30);
        ...
        auto drawPill3 = [&](int px, int pw, const char* label, const char* val, int col, bool sel) {
            if (sel) {
                ...
                canvas.fillRoundRect(px, fy2, pw, fh, 4, COL_PANEL);
