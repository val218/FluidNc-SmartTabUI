...
private:
    ...
    void drawDROScreen() {
        ...
        if (simJobRunning || (!simPath.empty() && simPathIdx > 0)) {
            ...
            // Work area map — always shown
            // Work area is rotated 90° CW: workY becomes screen-X, workX becomes screen-Y
            // So screen-X = 0..VIZ_W maps to machine Y: 0..workY
            //    screen-Y = 0..VIZ_H maps to machine X: 0..workX (inverted Y)
            int pad3 = 4;
            int mapW = VIZ_W - 2*pad3;
            int mapH = VIZ_H - 2*pad3;
            int mapX = VIZ_X + pad3;
            int mapY = VIZ_Y + pad3;

            // Scale: fit work area inside map area with aspect ratio
            float scaleX = (float)mapW / _workY;  // screen-X per mm of machine-Y
            float scaleY = (float)mapH / _workX;  // screen-Y per mm of machine-X
            float scale3 = std::min(scaleX, scaleY);
            int drawnW = (int)(_workY * scale3);
            int drawnH = (int)(_workX * scale3);
            ...
            // Grid lines (every 25% of work area)
            for (int gi = 1; gi < 4; gi++) {
                int gx = offX + drawnW * gi / 4;
                int gy = offY + drawnH * gi / 4;
                canvas.drawFastVLine(gx, offY, drawnH, COL_BORDER);
                canvas.drawFastHLine(offX, gy, drawnW, COL_BORDER);
            }

            // Home corner marker (small triangle)
            // homeCorner: 0=BL 1=BR 2=TL 3=TR
            // After 90CW rotation: BL=screen bottom-left, BR=bottom-right, TL=top-left, TR=top-right
            // Machine X=0 → screen-Y=top (offY+drawnH for BL/BR, offY for TL/TR)
            // Machine Y=0 → screen-X=left (offX for BL/TL, offX+drawnW for BR/TR)
            int hx = (_homeCorner==1||_homeCorner==3) ? offX+drawnW : offX;
            int hy = (_homeCorner==2||_homeCorner==3) ? offY         : offY+drawnH;
            canvas.fillCircle(hx, hy, 4, GREEN);
            canvas.drawCircle(hx, hy, 4, COL_WHITE);

            // Real-time position dot
            float px = simMode_active() ? (float)simMode_getPos(0) : (float)myAxes[0];
            float py = simMode_active() ? (float)simMode_getPos(1) : (float)myAxes[1];
            // Clamp to work area
            px = std::max(0.0f, std::min(px, (float)_workX));
            py = std::max(0.0f, std::min(py, (float)_workY));
            // Map: machine X → screen-Y (inverted from home corner), machine Y → screen-X
            int dotX, dotY;
            if (_homeCorner == 0) { // BL: X+ goes up, Y+ goes right
                dotX = offX + (int)(py * scale3);
                dotY = offY + drawnH - (int)(px * scale3);
            } else if (_homeCorner == 1) { // BR: X+ goes up, Y+ goes left
                dotX = offX + drawnW - (int)(py * scale3);
                dotY = offY + drawnH - (int)(px * scale3);
            } else if (_homeCorner == 2) { // TL: X+ goes down, Y+ goes right
                dotX = offX + (int)(py * scale3);
                dotY = offY + (int)(px * scale3);
            } else { // TR: X+ goes down, Y+ goes left
                dotX = offX + drawnW - (int)(py * scale3);
                dotY = offY + (int)(px * scale3);
            }
            canvas.fillCircle(dotX, dotY, 4, ORANGE);
            canvas.drawCircle(dotX, dotY, 4, COL_WHITE);

            // Dimension labels
            canvas.setFont(&fonts::Font0);
            canvas.setTextColor(COL_DIM);
            canvas.setTextDatum(bottom_left);
            char wdim[20]; snprintf(wdim, sizeof(wdim), "%dx%d", _workY, _workX);
            canvas.drawString(wdim, offX+2, offY+drawnH-2);
        }
...
public:
    ...
    void onDROChange()    override { reDisplay(); }  // always redraw — position dot updates on all tabs
    ...
    void onEncoder(int delta) override {
        ...
        if (_tab == 0 && _barSel != 0) {
            if (_barSel == 1) {
                ...
                // Rapid override: 0x93=+10%, 0x94=-10%
                fnc_realtime(delta>0 ? (realtime_cmd_t)0x93 : (realtime_cmd_t)0x94);
            } else {
                // Spindle override: 0x9A=+10%, 0x9B=-10%
