...
void readMpgSwitches() {
    ...
    // Axis switch — GPIO17(X), GPIO35(Y), GPIO21(Z), GPIO22(A), NC=off
    // GPIO21/22 are now dedicated switch inputs (encoder moved to GPIO16/4)
    // All pins are simple clean reads, no conflict with PCNT
    ...
    // Step switch — GPIO22(1mm), GPIO21(10mm), NC=0.1mm
    // Only active when an axis is selected
    if (mpgAxis >= 0) {
        bool s22 = (gpio_get_level(GPIO_NUM_22) == 0);
        bool s21 = (gpio_get_level(GPIO_NUM_21) == 0);
        if      (s21) mpgStepIdx = 2;   // 10mm
        else if (s22) mpgStepIdx = 1;   // 1mm
        else          mpgStepIdx = 0;   // 0.1mm
    }

    // Clear PCNT backlog when axis switches from off → active
