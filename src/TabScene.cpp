...
// Sample a GPIO N times and return true only if majority of reads are LOW.
// GPIO21/22 are shared with PCNT encoder - transient LOWs from encoder pulses
// are filtered out because they last only microseconds while a switch closure
// holds LOW for the full sample window (~200us).
static bool stableLow(gpio_num_t pin, int samples = 8) {
    int low = 0;
    for (int i = 0; i < samples; i++) {
        if (gpio_get_level(pin) == 0) low++;
        ets_delay_us(25);  // 25us between samples = 200us total window
    }
    return low >= (samples * 3 / 4);  // 75% majority
}

void readMpgSwitches() {
    // Throttle to once every 100ms — switch polling doesn't need to be fast
    ...
    // Axis switch — GPIO21/22 shared with encoder, use majority sampling
    // Priority: check non-conflicting pins first (17, 35), then shared (21, 22)
    bool a17 = (gpio_get_level(GPIO_NUM_17) == 0);   // X — dedicated pin, single read ok
    bool a35 = (gpio_get_level(GPIO_NUM_35) == 0);   // Y — input-only, single read ok
    bool a21 = stableLow(GPIO_NUM_21);                // Z — shared with encoder B
    bool a22 = stableLow(GPIO_NUM_22);                // A — shared with encoder A

    // Only one switch position is active at a time; first match wins
    if      (a22) mpgAxis = 3;   // A
    else if (a21) mpgAxis = 2;   // Z
    else if (a35) mpgAxis = 1;   // Y
    else if (a17) mpgAxis = 0;   // X
    else          mpgAxis = -1;  // off — step switch inactive

    // Step switch (only meaningful when axis is selected)
    if (mpgAxis >= 0) {
    ...
    // When axis changes from off → selected, clear PCNT backlog
    // so no queued counts fire as sudden jog burst
    if (prevAxis < 0 && mpgAxis >= 0) {
        pcnt_counter_clear(PCNT_UNIT_0);
    }

    // Trigger redisplay when state changes
