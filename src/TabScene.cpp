...
void readMpgSwitches() {
    ...
    // MacroBtn mode: P6 rising edge fires a macro
    if (_enableMode == 3) {
        bool p6Edge = (enable && !_p6Prev);
        if (p6Edge && current_scene) {
            // Fire enable macro — handled via schedule_action
            extern std::vector<MacroBtn> MACROS;
