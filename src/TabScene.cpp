...
private:
    ...
    void drawHeader() {
        ...
        // Right-side indicators — laid out from right edge inward, 22px each
        // RX (rightmost)
        {
            ...
            int txCol=txOn?ORANGE:COL_DIM, rxCol=rxOn?GREEN:COL_DIM;
            canvas.fillCircle(W-18, 10, 3, rxCol);
            hdrTxt("RX", W-14, 10, rxCol, middle_left);
            canvas.fillCircle(W-40, 10, 3, txCol);
            hdrTxt("TX", W-36, 10, txCol, middle_left);
        }
        // EN indicator
        {
            int enCol = mpgEnable ? GREEN : COL_DIM;
            canvas.fillCircle(W-62, 10, 3, enCol);
            hdrTxt("EN", W-58, 10, enCol, middle_left);
        }
        // E-stop badge (only when active)
        if (mpgEstopActive) {
            fillR(W-94, 2, 30, 16, 3, 0x4000);
            strokeR(W-94, 2, 30, 16, 3, RED);
            hdrTxt("STP", W-79, 10, RED);
        }
