...
private:
    ...
    void drawDROScreen() {
        ...
        // Always draw work area map as base layer
        {
            ...
            float scaleX=(float)mapW/_workY, scaleY=(float)mapH/_workX;
            float scale3=std::min(scaleX,scaleY);
            int drawnW=(int)(_workY*scale3), drawnH=(int)(_workX*scale3);
            ...
            // Position dot — always visible, uses work coords
            float px2=simMode_active()?(float)simMode_getPos(0):(float)myAxes[0];
            float py2=simMode_active()?(float)simMode_getPos(1):(float)myAxes[1];
            px2=std::max(0.0f,std::min(px2,(float)_workX));
            py2=std::max(0.0f,std::min(py2,(float)_workY));
            int dotX,dotY;
            if(_homeCorner==0){dotX=offX+(int)(py2*scale3);dotY=offY+drawnH-(int)(px2*scale3);}
            else if(_homeCorner==1){dotX=offX+drawnW-(int)(py2*scale3);dotY=offY+drawnH-(int)(px2*scale3);}
            else if(_homeCorner==2){dotX=offX+(int)(py2*scale3);dotY=offY+(int)(px2*scale3);}
            else{dotX=offX+drawnW-(int)(py2*scale3);dotY=offY+(int)(px2*scale3);}
            ...
            canvas.drawString(wdim,offX+2,offY+drawnH-1);
        }
