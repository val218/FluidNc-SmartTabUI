...
void readMpgSwitches() {
    ...
    // Initialise lastMoveTime to a future value so GPIO21/22 are distrusted
    // for the first 2 seconds after boot (PCNT settling time)
    static uint32_t lastMoveTime = 0xFFFFFFFF - 2000;

    uint32_t now = millis();
    if (now - lastRead < 80) return;
    lastRead = now;

    // Track encoder movement — if wheel moved recently, GPIO21/22 readings
    // for Z/A axis are unreliable (PCNT pulses look like switch closures)
    int16_t curPcnt = 0;
    ...
    // Consider encoder "still" only if no movement for 300ms
    bool encoderStill = (now - lastMoveTime) > 300;

    int prevAxis = mpgAxis, prevStep = mpgStepIdx;

    // ── Axis switch ──────────────────────────────────────────────────────────
    // GPIO17(X) and GPIO35(Y) are clean dedicated pins — always readable
    // GPIO21(Z) and GPIO22(A) shared with PCNT — only trust when encoder is still
    bool a17 = (gpio_get_level(GPIO_NUM_17) == 0);
    bool a35 = (gpio_get_level(GPIO_NUM_35) == 0);
    // For Z/A: only update if encoder has been still for 300ms
    // Otherwise keep the last known axis value to prevent jumping
    bool a21 = encoderStill ? (gpio_get_level(GPIO_NUM_21) == 0) : (mpgAxis == 2);
    bool a22 = encoderStill ? (gpio_get_level(GPIO_NUM_22) == 0) : (mpgAxis == 3);

    if      (a22) mpgAxis = 3;   // A
    else if (a21) mpgAxis = 2;   // Z
    else if (a35) mpgAxis = 1;   // Y  (add 10k external pullup GPIO35→3.3V)
    else if (a17) mpgAxis = 0;   // X
    else          mpgAxis = -1;  // off

    // ── Step switch ──────────────────────────────────────────────────────────
    // GPIO16(1mm) and GPIO4(10mm) — dedicated pins, always clean
    if (mpgAxis >= 0) {
        bool s4  = (gpio_get_level(GPIO_NUM_4)  == 0);
        bool s16 = (gpio_get_level(GPIO_NUM_16) == 0);
        if      (s4)  mpgStepIdx = 2;
        else if (s16) mpgStepIdx = 1;
        else          mpgStepIdx = 0;
    }

    // Clear PCNT backlog when axis goes off → active (prevents jog burst)
    if (prevAxis < 0 && mpgAxis >= 0) {
        pcnt_counter_clear(PCNT_UNIT_0);
        lastPcnt     = 0;
        lastMoveTime = 0;   // reset so first read after selection is trusted
    }
