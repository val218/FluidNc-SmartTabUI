...
static void init_capacitive_cyd() {
    ...
    // MPG step/axis switches on GPIO4,16,17,35 (shared with RGB LED pins — LED not driven)
    // GPIO21/22 shared with PCNT encoder — PCNT configures them, no pinMode needed
    // GPIO35 is input-only, no internal pullup available
    // NOTE: GPIO6-11 are internal flash SPI — never use as GPIO
    pinMode(GPIO_NUM_4,  INPUT_PULLUP);
    pinMode(GPIO_NUM_16, INPUT_PULLUP);
    pinMode(GPIO_NUM_17, INPUT_PULLUP);
    // GPIO35: input-only, no pullup — relies on external pullup on switch wiring
