...
static void init_capacitive_cyd() {
    ...
    // MPG encoder — original wiring: A=GPIO22, B=GPIO21
    enc_a = GPIO_NUM_22;
    enc_b = GPIO_NUM_21;

    // Step switch: pos2=GPIO16(1mm), pos3=GPIO4(10mm) — no PCNT conflict
    // Axis switch: pos2=GPIO17(X), pos3=GPIO35(Y), pos4=GPIO21(Z), pos5=GPIO22(A)
    // GPIO21/22 shared with PCNT — axis Z/A filtered via stableLow() in software
    // GPIO35 is input-only, no internal pullup — must use external 10k pullup to 3.3V
