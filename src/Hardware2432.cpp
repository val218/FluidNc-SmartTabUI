...
static void init_capacitive_cyd() {
    ...
    // MPG handwheel encoder — dedicated pins, no conflicts
    enc_a = GPIO_NUM_16;
    enc_b = GPIO_NUM_4;

    // E-stop button — GPIO17, active-LOW, internal pullup
    // Connect one wire to GPIO17, other wire to GND
    pinMode(GPIO_NUM_17, INPUT_PULLUP);

    // PCF8574 I2C GPIO expander for MPG rotary switches
    // I2C on GPIO21(SDA) and GPIO22(SCL) — dedicated bus, no conflict with touch (GPIO33/32)
    // Address 0x20 (A0=A1=A2=GND)
    // P0=Axis X, P1=Axis Y, P2=Axis Z, P3=Axis A
    // P4=Step 1mm, P5=Step 10mm, P6=unused, P7=unused
    // All active-LOW — switch common to GND, position wire to PCF8574 pin
    {
        i2c_config_t conf = {};
        ...
        // Set all PCF8574 pins HIGH (input mode)
        uint8_t val = 0xFF;
        i2c_master_write_to_device(I2C_NUM_1, 0x20, &val, 1, pdMS_TO_TICKS(10));
    }
