...
static void mpgTask(void*) {
    ...
    for (;;) {
        ...
        if (!estopHigh && lastEstop) {
            // E-stop PRESSED:
            // 1. FeedHold to stop motion cleanly
            // 2. After 200ms send Soft Reset (0x18) → FluidNC reboots into Alarm state
            extern volatile bool _forceAlarm;
            _forceAlarm = true;
            fnc_realtime((realtime_cmd_t)'!');  // '!' Feed Hold
            fnc_term_inject("> E-STOP: Hold");
            resumeTime = now + 200;             // reuse resumeTime as reset-pending timer
        }

        if (estopHigh && !lastEstop) {
            // E-stop RELEASED:
            // Wait 600ms for FluidNC to finish booting after reset, then send $X
            resumeTime = now + 600;
            fnc_term_inject("> E-STOP released: waiting...");
        }
        ...
        if (resumeTime && now >= resumeTime) {
            resumeTime = 0;
            if (!estopHigh) {
                // Still pressed — send the Soft Reset now
                fnc_realtime((realtime_cmd_t)0x18);  // Ctrl-X Soft Reset → Alarm
                fnc_term_inject("> E-STOP: Reset → Alarm");
            } else {
                // Released — send $X to clear Alarm → Idle
                extern volatile bool _forceAlarm;
                _forceAlarm = false;
                send_line("$X");
                fnc_term_inject("> $X sent: Alarm cleared → Idle");
