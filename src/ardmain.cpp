...
static void mpgTask(void*) {
    ...
    for (;;) {
        ...
        if (estopHigh != lastEstop) { extern volatile bool _mpgChanged; _mpgChanged = true; }
        if (!estopHigh && lastEstop) {
            // E-stop pressed: show overlay immediately, send FeedHold + Reset
            extern volatile bool _forceAlarm;
            _forceAlarm = true;
            fnc_realtime((realtime_cmd_t)'!');  // Feed Hold = 0x21
            fnc_term_inject("> E-STOP: FeedHold sent");
            resetPending = true;
            resetTime    = now;
        }
        if (estopHigh && !lastEstop) {
            // E-stop released: clear overlay after unlock
            unlockTime = now + 500;
        }
        lastEstop = estopHigh;
        ...
        if (resetPending && now - resetTime >= 100) {
            resetPending = false;
            fnc_realtime((realtime_cmd_t)0x18);  // Ctrl-X = Soft Reset
