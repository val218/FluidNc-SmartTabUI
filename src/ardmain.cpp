...
static void runSettingsMenu(AppSettings& s) {
    ...
    for (;;) {
        // ── Keep UART alive while in settings — drain FluidNC serial buffer ──
        // Without this, FluidNC fills the 256-byte RX buffer → XON/XOFF stalls
        // → pendant stops receiving status reports → connection timeout
        if (!simMode_active()) {
            fnc_poll();  // drain UART + parse any incoming bytes
        }
...
void setup() {
    ...
    // UART reader task: Core 0, priority 2 (higher than mpgTask priority 1).
    ...
    extern void uart_reader_task(void*);
