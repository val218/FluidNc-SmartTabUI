...
void setup() {
    ...
    drawProgress(62, "Loading settings...", 0x065F);
    AppSettings s;
    settings_load(s);
    drawProgress(68, "Applying config...", 0x065F);
    ...
    // Step 3: Wait for FluidNC — show warning immediately if not responding
    {
        // Send status request and wait up to 2s for first real response
        // A real response means state transitions out of Disconnected
        drawProgress(80, "Connecting to FluidNC...", 0x065F);
        request_status_report();

        auto drawNotConnected = [&]() {
            // Draw warning over current logo screen
            display.fillRoundRect(20, 68, W-40, 104, 8, 0x1082);
            display.drawRoundRect(20, 68, W-40, 104, 8, 0xF800);
            display.setFont(&fonts::Font2);
            display.setTextDatum(middle_center);
            display.setTextColor(0xF800);
            display.drawString("NOT CONNECTED", W/2, 90);
            display.setFont(&fonts::Font0);
            display.setTextColor(0x9D17);
            display.drawString("FluidNC not responding", W/2, 108);
            display.drawString("Check UART / power cable", W/2, 120);
            display.drawString("Waiting for connection...", W/2, 134);
        };
        ...
        while (state == Disconnected) {
            ...
            if (!warningShown && elapsed > 1500) {
                // No response in 1.5s — show warning immediately
                warningShown = true;
                drawNotConnected();
            }
            ...
            if (warningShown) {
                // Animate dots
                int dot = (elapsed / 400) % 4;
                const char* dots[] = {"   ", ".  ", ".. ", "..."};
                display.setFont(&fonts::Font0);
                display.setTextDatum(middle_center);
                display.fillRect(W/2-30, 143, 60, 9, 0x1082);
                display.setTextColor(0xFD20);
                display.drawString(dots[dot], W/2, 148);
            } else {
                // Still within grace period — show progress
                int pct = 80 + (int)(elapsed * 10 / 1500);
                drawProgress(std::min(pct, 90), "Connecting to FluidNC...", 0x065F);
            }
            delay(50);
        }

        if (warningShown) {
            // Was disconnected, now connected — brief success flash
            display.fillRoundRect(20, 68, W-40, 104, 8, 0x0841);
            display.drawRoundRect(20, 68, W-40, 104, 8, 0x07E0);
            display.setFont(&fonts::Font2);
            display.setTextDatum(middle_center);
