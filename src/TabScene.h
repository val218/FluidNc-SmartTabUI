...
bool   mpgEnableHeld();
bool   mpgConsumeChanged();  // true if MPG state changed since last call
void   mpgSignalChanged();    // signal redraw needed (from other tasks)   // true when P6 enable button is held
void   mpgCheckMacroFire();
void   mpgSetEstop(bool pressed);  // called from mpgTask to update e-stop display // call from main loop to fire P6 macro if pending
