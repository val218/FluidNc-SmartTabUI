...
static uint32_t _lastRedraw = 0;
static const uint32_t REDRAW_INTERVAL_MS = 33;  // ~30fps max

void markDirty() { _displayDirty = true; }

void flushDisplay() {
    if (!_displayDirty) return;
    uint32_t now = millis();
    if (now - _lastRedraw < REDRAW_INTERVAL_MS) return;
    _lastRedraw = now;
    _displayDirty = false;
...
void dispatch_touch() {
    ...
    // Debounce: 120ms minimum between click/hold events
    // Prevents double-fire from gloves, vibration, or capacitive noise
    static uint32_t _lastClickMs = 0;
    bool isClick = t.wasClicked() || t.wasHold();
    if (isClick) {
        uint32_t now = millis();
        if (now - _lastClickMs < 120) return;
        _lastClickMs = now;
    }
...
extern uint32_t g_pressExpiryMs;
extern void tabui_checkPressExpiry();
void dispatch_events() {
    ...
    // Flush display once per loop — rate-limited to 30fps
    flushDisplay();

    // Clear press highlights after 300ms (home + macro button feedback)
    tabui_checkPressExpiry();
}
