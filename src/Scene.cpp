...
void dispatch_touch() {
    // Enable button (PCF8574 P6) must be held for touch to work
    // In simulation mode, always allow touch (no physical button)
    if (!simMode_active() && !mpgEnableHeld()) {
        touch.update(millis());  // keep touch state machine running but discard input
        return;
    }
